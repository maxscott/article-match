class Similizer
  def cosine_similarity arr_of_name_vals1, arr_of_name_vals2
    ...
    #keywords1.filter { |pair| intersect_names.any? { |name| name == pair.name} }
    #keywords2.filter { |pair| intersect_names.any? { |name| name == pair.name} }
    
    keywords1.sort! { |h1,h2| h1[:name] <=> h2[:name] }
    keywords2.sort! { |h1,h2| h1[:name] <=> h2[:name] }

    dot( keywords1.map { |h| h[:val] }, keywords2.map { |h| h[:val] } )
  end
